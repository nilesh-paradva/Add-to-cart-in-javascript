...
const displayCart = () => {
    ...
    selectData.innerHTML = storage.map(item =>  //SelectProduct data print in cart box item.keyname data print 
        `<tr>
            ...
            <td><a href="javascript:void(0)" class="btn btn-danger" onclick="deleteFromCart(${item.id})"><i class="fa-solid fa-trash-can"></i></a></td>
            <td><a href="view.html?id=${item.id}" target="_blank" class="btn btn-primary px-3 py-2">View ALL</a></td>
        </tr>`
    ).join('');
    selectCount();
    updateTotalPrice();
